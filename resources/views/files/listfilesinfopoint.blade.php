    ...
    <div class="layout-px-spacing">
        ...
        <div class="row mt-4">
            <div class="col-xl-6 col-lg-6 col-sm-6">
                <h1 class="text-primary">AGGIUNGI FILE ALL'INFOPOINT</h1>
                <h2 class="text-primary">{{ $infopoint->denominazione }}</h2>
            </div>
        </div>         
            <div class="row layout-spacing mt-4">
                <div class="col-lg-12">
                    <h2 class="text-center">Lista files per infopoint: {{ $infopoint->denominazione }}</h2>
                            ...
                            <table id="style-3" class="table style-3 table-hover dataTable no-footer" role="grid"
                                ...
                                <thead>
                                    <tr role="row">
                                        ...
                                        <th class="sorting" tabindex="0" aria-controls="style-3" rowspan="1" colspan="1"
                                            ...
                                            Descrizione File</th>
                                            <th class="d-none" tabindex="0">note</th>
                                            <th class="d-none" tabindex="0">gruppoid</th>
                                            <th class="d-none" tabindex="0">accessoid</th>
                                ...
                                <tbody>
                                    <tr role="row" class="files-item">
                                        @foreach ($filesinfopoint as $fileinfopoint)
                                        <td class="checkbox-column text-center sorting_1">{{ $fileinfopoint->id }}</td>
                                        <td>{{ $fileinfopoint->nomeFile }}</td>
                                        <td>{{ $fileinfopoint->estensioneFile }}</td>
                                        <td>{{ $fileinfopoint->descrizioneFile }}</td>
                                        <td class="d-none">{{ $fileinfopoint->note }}</td>
                                        <td class="d-none">{{ $fileinfopoint->gruppo_id }}</td>
                                        <td class="d-none">{{ $fileinfopoint->livelloaccesso_id }}</td>
                                        <td class="text-center"><span
                                                class="shadow-none badge" style="color:#fff; background-color: #{{ $gruppofiles->find($fileinfopoint->gruppo_id, "id")->codcolore }}">{{ $gruppofiles->find($fileinfopoint->gruppo_id, "id")->descrizione }}</span></td>
                                        <td class="text-center">
                                            <ul class="table-controls">
                                                ...
                                                <li><a href="{{ asset($fileinfopoint->pathinfopointfiles) }}" target="_new" class="bs-tooltip"
                                                ...
                                                <li><a href="{{ asset($fileinfopoint->pathinfopointfiles) }}" download rel="noopener noreferrer" target="_blank"  class="bs-tooltip"
                                                        data-toggle="tooltip" data-placement="top" title=""
                                                ...
                                                <li><a href="{{  route('filesinfopoint.destroy', $fileinfopoint->id) }}" class="bs-tooltip delete-file-a"
                                                        data-toggle="tooltip" data-placement="top" title=""
        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content">
             ...
             <div class="modal-body">
                 <form id="modalAddFileForm" action="{{ route('filesinfopoint.store') }}" method="POST" enctype="multipart/form-data">
                     <input type="hidden" name="infopoint_id" value="{{ $infopoint->id }}">
                     ...
                     <div class="form-row">
                        <div class="col-md-12 mb-4">
                            <label for="accessoFile">Livello Accesso</label>
                            <select id="accessoFile" name="accessoFile" class="form-control">
                                <option value="" selected>Seleziona...</option>
                                @foreach ($livelliaccesso as $livelloaccesso)
                                    <option value="{{ $livelloaccesso->id }}">{{ $livelloaccesso->descrizione }}</option>
                                @endforeach
                            </select>
                        </div>
                    </div>
                     <div class="form-row">
                         <div class="col-md-6">
                             <label for="caricaFileInfopoint">Carica il file</label>
                             <input type="file" name="caricaFileInfopoint" class="form-control-file" id="caricaFileInfopoint">
                         </div>

                     </div>
                     <div class="form-row">
                        <div class="col-md-12 mb-4">
                            <label for="note">Note Aggiuntive File</label>
                            <textarea type="text" class="form-control" name="note" id="note"
                                placeholder="Inserisci una nota del file" rows="3"></textarea>
                        </div>
                    </div>
 ...
 <div class="modal fade" id="modalModFile" tabindex="-1" role="dialog" aria-labelledby="modalModFileLabel" aria-hidden="true">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
         ...
         <div class="modal-body">
            @isset($fileinfopoint->id)
             <form id="modalModFileForm" enctype="multipart/form-data">
                 <input type="hidden" name="_token" value="{{ csrf_token() }}">
                 <input type="hidden" name="id-file" id="id-file" value="{{ $fileinfopoint->id }}">
                 {{ method_field('PUT') }}
                 <div class="form-row">
                     <div class="col-md-12 mb-4">
                         ...
                         <textarea type="text" class="form-control" name="descrizioneFileMod" id="descrizioneFileMod"
                             placeholder="Inserisci una descrizione del contenuto del file" rows="3">{{ $fileinfopoint->descrizioneFile?$fileinfopoint->descrizioneFile:'' }}</textarea>
                     </div>
                 </div>
                 <div class="form-row">
                     <div class="col-md-12 mb-4">
                         ...
                         <select id="gruppoFileMod" name="gruppoFileMod" class="form-control">
                             <option value="" selected>Seleziona...</option>
                             @foreach ($gruppofiles as $gruppofile)
                                 <option value="{{ $gruppofile->id }}" {{ $gruppofile->id == $fileinfopoint->gruppo_id ? 'selected' : '' }}>{{ $gruppofile->descrizione }}</option>
                             @endforeach
                         </select>
                     </div>
                 </div>
                 <div class="form-row">
                    <div class="col-md-12 mb-4">
                        <label for="livelloAccessoMod">Livello di Accesso</label>
                        <select id="livelloAccessoMod" name="livelloAccessoMod" class="form-control">
                            <option value="" selected>Seleziona...</option>
                            @foreach ($livelliaccesso as $livelloaccesso)
                                <option value="{{ $livelloaccesso->id }}" {{ $livelloaccesso->id == $fileinfopoint->livelloaccesso_id ? 'selected' : '' }}>{{ $livelloaccesso->descrizione }}</option>
                            @endforeach
                        </select>
                    </div>
                </div>
                <div class="form-row">
                    <div class="col-md-12 mb-4">
                        <label for="noteMod">Note Aggiuntive File</label>
                        <textarea type="text" class="form-control" name="noteMod" id="noteMod"
                            placeholder="Inserisci una nota del file" rows="3"></textarea>
                    </div>
                </div>
                 <div class="modal-footer">
...
<script>
    ...
    $(document).ready(function(){
        $('.editbtn').on('click', function(){
            ...
            $('#noteMod').val(data[4]);
            $('#gruppoFileMod').val(data[5]);
            $('#livelloAccessoMod').val(data[6]);
        });
