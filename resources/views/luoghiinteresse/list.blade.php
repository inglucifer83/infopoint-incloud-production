                ...
                <table id="individual-col-search" class="table table-hover">
                    <thead>
                        <tr>
                            ...
                            <th>Indirizzo</th>
                            <th>Tipologia</th>
                            <th class="text-center dt-no-sorting">Costo Biglietto</th>
                            <th class="text-center dt-no-sorting">Mappa</th>
                            <th class="text-center dt-no-sorting">Azioni</th>
                        </tr>
                    </thead>
                    <tbody>
                        @foreach ( $luoghiinteresse as $luogo)
                        <tr class="luogo-item">
                            <td>{{ Str::words($luogo->denominazione, 5, ' ...') }}</td>
                            <td>{{ $comuni->find($luogo->comune_id, "id")->nomecomune }}</td>
                            <td>{{ $luogo->frazione }}</td>
                            <td>{{ Str::words($luogo->indirizzo, 4, ' ...') }}</td>
                            <td>{{ $tipoluoghiinteresse->find($luogo->tipoluoghiinteresse_id, "id")->descrizione }}</td>
                            <td>{{ $luogo->costobigliettoadulti }}</td>
                            <td><a href="{{ $luogo->urlmappa }}" target="_new" alt="Mappa">Link</a></td>
                            <td class="text-center">
                                <a href="{{ route('luoghiinteresse.show', $luogo->id) }}" target="_parent" class="bs-tooltip" data-toggle="tooltip" data-placement="top" title="" data-original-title="Apri"><svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="feather feather-eye"><path d="M1 12s4-8 11-8 11 8 11 8-4 8-11 8-11-8-11-8z"></path><circle cx="12" cy="12" r="3"></circle></svg></a>
                                @if(Auth::user()->isAdmin() || Auth::user()->getUserInfopointId() === $luogo->comune_id)
                                    <a href="{{ route('luoghiinteresse.edit', $luogo->id) }}" class="bs-tooltip" data-toggle="tooltip" data-placement="top" title="" data-original-title="Modifica"><svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="feather feather-edit"><path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"></path><path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z"></path></svg></a>
                                @endif
                                ...
                                @if(Auth::user()->isAdmin() || Auth::user()->getUserInfopointId() === $luogo->comune_id)
                                    <a href="{{ route('luoghiinteresse.destroy', $luogo->id) }}" class="bs-tooltip delete-luogo-a" data-toggle="tooltip" data-placement="top" title="" data-original-title="Elimina"><svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="feather feather-x-circle table-cancel"><circle cx="12" cy="12" r="10"></circle><line x1="15" y1="9" x2="9" y2="15"></line><line x1="9" y1="9" x2="15" y2="15"></line></svg></a>
                                @endif
                            </td>
                        </tr>
                        @endforeach
                    ...
                    <tfoot>
                        <tr>
                            ...
                            <th>Indirizzo</th>
                            <th>Tipologia</th>
                            <th class="invisible">Costo Biglietto</th>
...
<script>
    $('document').ready(function(){
              ...
              $('.delete-luogo-a').on('click', function(ele){
                  ele.preventDefault();
                  var urlApp = $(this).attr('href');
                  var tr = $(this).parents('.luogo-item');
