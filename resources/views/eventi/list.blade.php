    ...
    <div class="layout-px-spacing">
        ...
        <div class="row mt-4">
            ...
            <div class="col-xl-6 col-lg-6 col-sm-6">
                <h1 class="text-primary">Elenco Eventi</h1>
            </div>
        </div>
        <div class="row mt-4">
            @if(!Auth::user()->isAdmin())
                <div class="col-xl-4 col-lg-6 col-sm-6 layout-spacing d-flex justify-content-center">
                    <a href="{{ route('eventi.create') }}?comune_id={{ Auth::user()->getUserInfopointId() }}"><button class="btn btn-info mb-2 mr-2 btn-lg"><svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="feather feather-sunrise"><path d="M17 18a5 5 0 0 0-10 0"></path><line x1="12" y1="2" x2="12" y2="9"></line><line x1="4.22" y1="10.22" x2="5.64" y2="11.64"></line><line x1="1" y1="18" x2="3" y2="18"></line><line x1="21" y1="18" x2="23" y2="18"></line><line x1="18.36" y1="11.64" x2="19.78" y2="10.22"></line><line x1="23" y1="22" x2="1" y2="22"></line><polyline points="8 6 12 2 16 6"></polyline></svg> Aggiungi Evento</button></a>
                </div>
            @endif
        </div>
        <div class="row layout-top-spacing">
            <div class="col-xl-12 col-lg-12 col-sm-12">
			    ...
			    <td>{!! date("d-m-Y", strtotime($evento->inizio_evento))  !!}</td>
                            <td>{!! date("d-m-Y", strtotime($evento->fine_evento))  !!}</td>
                            <td>{{ Str::words($evento->denominazione, 4, ' ...') }}</td>
                            <td>{{ $comuni->find($evento->comune_id, "id")->nomecomune }}</td>
                            <td>{{ $evento->frazione }}</td>
                            <td>{{ Str::words($evento->indirizzo, 3, ' ...') }}</td>
                            <td>{{ $tipoeventi->find($evento->tipoeventi_id, "id")->descrizione }}</td>
                            <td class="text-center">
                                <a href="{{ route('eventi.show', $evento->id) }}" target="_parent" class="bs-tooltip" data-toggle="tooltip" data-placement="top" title="" data-original-title="Apri"><svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="feather feather-eye"><path d="M1 12s4-8 11-8 11 8 11 8-4 8-11 8-11-8-11-8z"></path><circle cx="12" cy="12" r="3"></circle></svg></a>
                                @if(Auth::user()->isAdmin() || Auth::user()->getUserInfopointId() === $evento->comune_id)
                                    <a href="{{ route('eventi.edit', $evento->id) }}" class="bs-tooltip" data-toggle="tooltip" data-placement="top" title="" data-original-title="Modifica"><svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="feather feather-edit"><path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"></path><path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z"></path></svg></a>
                                @endif
...
<script>
    $('document').ready(function(){
              ...
              $('.delete-evento-a').on('click', function(ele){
                  ele.preventDefault();
                  var urlApp = $(this).attr('href');
                  var tr = $(this).parents('.evento-item');
